...
import com.applock.lockview.GestureLockView.Mode;
...
public class GestureLockViewGroup extends RelativeLayout
{
	 ...
	 * 匹配成功的颜色
	 */
	private int mMatchColor = 0xFF6AC239;
	...
	public GestureLockViewGroup(Context context, AttributeSet attrs,
		...
		for (int i = 0; i < n; i++)
		{
			...
			case R.styleable.GestureLockViewGroup_color_finger_up:
				...
				break;
			case R.styleable.GestureLockViewGroup_color_match:
				mMatchColor = a.getColor(attr, mMatchColor);
				break;
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
	{
		...
		initGestureLockViews();
	}

	/**
	 * 初始化mGestureLockViews
	 */
	public void initGestureLockViews() {
		if (mGestureLockViews == null)
		{
			...
			Log.e("mydebug", "------ 0  mFingerUpColor  " + mFingerUpColor);
...
//			mPaint.setAlpha(50);
			this.mTryTimes--;
			Log.e("mydebug", "------1 mFingerUpColor  " + mFingerUpColor);
			// 回调是否成功
			if (mOnGestureLockViewListener != null && mChoose.size() > 0)
			{
				...
				if(matched){
					mPaint.setColor(mMatchColor);
					mPaint.setAlpha(50);
					for(int i = 0; i < mGestureLockViews.length; ++i){
						mGestureLockViews[i].setmColorFingerUp(mMatchColor);
					}
				}
				else{
					mPaint.setColor(mFingerUpColor);
				...
				Log.e("mydebug", "------ 2 mFingerUpColor  " + mFingerUpColor);
				if (this.mTryTimes == 0)
				{
	...
	{
		if ((mAnswer == null) || (mAnswer.length != mChoose.size()))
			return false;

		for (int i = 0; i < mAnswer.length; i++)
		{
			Log.e("mydebug", "----  choose  " + mChoose.get(i));
			Log.e("mydebug", "----  answer  " + mAnswer[i]);
...
//	}
	public int[] getChoose(){
		...
		int[] choose = new int[mChoose.size()];
		for( int i = 0; i < mChoose.size(); ++i){
			choose[i] = mChoose.get(i);
		}
		return choose;
	}
